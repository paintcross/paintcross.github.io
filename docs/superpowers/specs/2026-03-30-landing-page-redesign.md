...
### Platform Cards
Three cards displayed in a horizontal row directly in the hero. All icons are flat SVGs with `fill="var(--text)"` for dark/light mode compatibility — no emoji.

| Card | Icon source | viewBox | Name | Sub-label |
|------|-------------|---------|------|-----------|
| macOS | Existing SVG in download section (Apple silhouette) | `0 0 32 32` | macOS | Intel + Apple Silicon |
| Windows | Existing SVG in download section (4-pane grid) | `0 0 32 32` | Windows | x64 Installer |
| Linux | SimpleIcons `linux.svg` (Tux penguin, CC0) | `0 0 24 24` | Linux | deb · rpm · AppImage |

SimpleIcons Linux SVG path (use as-is, `fill="var(--text)"`):
```
